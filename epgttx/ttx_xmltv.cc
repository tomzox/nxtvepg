 ...
 */
bool XMLTV::ImportXmltvFile(const char * fname)
{
 ...
 */
bool XMLTV::ExportXmltv(list<TV_SLOT>& NewSlots, const char * p_file_name,
                        const char * p_my_ver, const char * p_my_url)
{
   ...
   string acq_date_str = GetXmlTimestamp(mp_db->page_db.get_acq_timestamp());
   if (!p_my_ver || !p_my_url) {
      fprintf(fp, "<tv date=\"%s\">\n", acq_date_str.c_str());
   }
   else {
      fprintf(fp, "<tv generator-info-name=\"%s\" generator-info-url=\"%s\" "
                  "source-info-name=\"Teletext %s\" "
                  "date=\"%s\">\n",
                  p_my_ver, p_my_url, m_ch_name.c_str(),
                  acq_date_str.c_str());
   }
