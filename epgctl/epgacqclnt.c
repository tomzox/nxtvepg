               ...
               case EPGDB_STATS_UPD_TYPE_INITIAL:
                  if (size == sizeof(pBody->stats_ind.u.initial))
                  {
                     if (clientState.endianSwap)
                     {
                        ...
                        static_assert(sizeof(pBody->stats_ind.u.initial.stats.pkgStats) % sizeof(uint32_t) == 0, "");
                        static_assert(sizeof(pBody->stats_ind.u.initial.stats.ttx_dec) % sizeof(uint32_t) == 0, "");
