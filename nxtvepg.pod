...
Since version 3.0, nxtvepg only supports vendor-provided WDM drivers.

Note if WDM drivers fail to work with nxtvepg, a third option may be to
run nxtvepg in parallel to TV-application "K!TV".  In this case the
...
button next to the entry field to open a directory selector dialog. You
can also edit the directory name directly and press the Return key to
update the list of files. (Note only files that have extension ".xml" and
contain valid XMLTV syntax are shown in the list.)
...
option is enabled only if you have a TV capture card and the teletext
grabber is configured (see L<"CONFIGURATION"> section "Teletext grabber").
