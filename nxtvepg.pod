...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.53 2001/09/16 10:47:19 tom Exp tom $
# ---------------------------------------------------------------------------
...
The only way to stop the process in this mode is to send it a signal, i.e.
kill it with SIGTERM. This can simply be done with:

   kill `cat /tmp/.vbi0.pid`

Note that it is not advisable to run the daemon while the GUI is in use,
because incoming data will not be forwarded from the daemon to the GUI
process (such a feature may be added in the future).  If you plan on using
the daemon on a regular basis, put a little wrapper script around nxtvepg
that automatically terminates the daemon when the GUI is started and
...
see L<"CONFIGURATION">). The second one are the timescale popup windows,
...
acquisition databases. (By default these are identical, but you can configure
background acquisition on multiple databases, see L<"ACQUISITION MODES">).
...
kind of information presented depends on the current state.
...
the browser database, and one for the acquisition database.

The timescale windows have one scale for each
...
The number of AI blocks received since acquisition start.
...
The average how often each block in the AI was received since start of the
acquisition.  Divided in Now, stream 1 and stream 2.
...
you can execute any command with parameters of the selected programme
(like title or start time) on its command line. The user commands and
can be added via the Configure menu (see L<"CONFIGURATION">).
...
Acquisition can also been switched on and off automatically by sending the
process the signal HUP. Note: the signal must be sent to the acquisition
slave process, the pid of which is stored in F</tmp/.vbi.pid> while the
device is in use.
...
variables the modifiers are currently ignored.

The substrings which replace the formal variables are enclosed in single
