...
#
# $Id: nxtvepg.pod,v 1.27 2001/02/14 21:26:41 tom Exp tom $
# ---------------------------------------------------------------------------
...
open the provider selection popup from the Configure menu and select
you favorite one.
...
If this did not help, you can still add the missing providers manually.
Therefore set the acquisition mode to I<passive>, then use
an external application to tune the channel (Windows users have to quit
nxtvepg first; then start the other application; then tune the channel;
then quit the TV application; finally start nxtvepg again). In passive
mode nxtvepg will not touch the tuner and infinitly wait for Nextview
reception on the current channel. On Windows and Linux with old bttv
drivers (version 0.7.50 and earlier) this method has the disadvantage
that no frequency will be known for this provider so that it can not
be updated automatically (see L<"DATA ACQUISITION"> for details).
...
data available depends entirely on the content provider.
...
menu, you will get a popup with two listboxes: the left one contains
...
The B<Configure> button in the popup gives you fine-control over the
...
date of last update etc.  See L<"Data Acquisition"> for details.


=item B<Acq statistics>

Toggles the acquisition statistics window, which informs you about
state and progress of acquisition. See L<"Data Acquisition"> for details.
...
If you leave the popup with Ok, the selected provider's database will be
...
channel for a VPS or PDC identification or en EPG package. Else
...
requencies are kept in the rc/ini file). This allows you to regain your
...
Open the network selection popup. It allows to permanently suppress
...
controlled channel changes (this will be reported, e.g. in the
status line at the bottom of the browser window, as I<forced passive>
acquisition mode).
