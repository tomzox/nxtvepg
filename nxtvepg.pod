...
done with grabbing EPG data for a network.
...
While the Teletext Grabber enabled and working on the same database, and
it is a merged database, a second pane is added to the window that shows a
histogram of acquisition progress.
...
The first and last teletext page number which are captured for grabbing
