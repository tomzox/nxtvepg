...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.37 2001/04/21 12:54:37 tom Exp tom $
# ---------------------------------------------------------------------------
...
If you're a user of the I<xawtv> TV application (UNIX only), you'll find
a I<Tune TV> button below the clock. When you press it, the network of
the currently selected programme will be tuned into xawtv. With the
right mouse button you can also pop up a small menu which offers basic
control over xawtv.
...
clock. These shortcuts enable you to invoke filters by a single click
...
from the Filter menu to save the setting. You will get a dialog
that enables you to change any names or the ordering, or to
delete shortcuts. If you want to discard all changes,
quit the dialog with the I<Abort> button. Else, use I<Save>.

The filter mask list in the dialog reflects which types of filters
...
you can also access the dialog from the I<Configure> menu.
...
the F<.xawtv> configuration file. This is required because I<xawtv-remote>
gets the network name as parameter when you press the I<"Tune TV">
button. If this name does not match exactly a name in F<.xawtv>,
xawtv will not be able to switch to the requested channel. For many
channels there will be no need to change anything, but for some there
exist many different variants, e.g. in Germany "Super RTL" vs. "S-RTL"
...
configured before, or if you haven't done so, the name that matches
F<.xawtv> best. Unmatched names are marked red. On the right side,
you're offered three ways to change the name: topmost is a simple
entry field where you can type in an arbitrary name. Below is a popup
menu which contains all network names defined in F<.xawtv>. Below
is a listbox with the original names used in the various databases.
When you select a name from the menu or the listbox, it will be
copied into the entry field and the network name list on the left.
If you want to save the changed list, leave the dialog with I<Save>,
else use I<Abort>.
