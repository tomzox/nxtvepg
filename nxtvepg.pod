...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.135 2009/03/14 21:24:32 tom Exp tom $
# ---------------------------------------------------------------------------
...
B<nxtvepgd> S<[ daemon options ]> S<[ database ]>

=head1 DESCRIPTION

B<nxtvepg>
is an X11 and Win32 application to receive, analyze and browse TV programme
schedules transmitted on top of Teletext as defined by the European
Telecommunications Standards Institute (ETSI) in ETS 300 707:
"Protocol for a TV Guide using electronic data transmission".

B<nxtvepgd>
is a stripped-down version of nxtvepg which only performs data acquisition
as a background daemon process.

The Nextview EPG standard was developed for use in TV sets, but the data can be
received and used in a PC, too - provided you have a Teletext capable TV tuner
card and are lucky enough to have a content provider in your country.

B<nxtvepg>
enables you to obtain free TV programme listings for all of the major
networks in Germany, Austria, France, Belgium and Switzerland. Currently
...
channels listed above, you can still use nxtvepg to browse XMLTV files.

Since version 2.8.0 nxtvepg also allows to extract EPG data from teletext
or import from external XMLTV sources. Note when importing data you need
to take care to respect copyright, same as when exporting data. In particular,
the German law does not permit "Web scraping", i.e. extraction of programme
information from Internet sources without prior permission of the content
provider.
...
UNIX only. Also only available when nxtvepg was configured at
compile time (i.e. in the Makefile) to use the I<ZVBI library>
(L<http://zapping.sourceforge.net/ZVBI/index.html>) for acquisition.

This option enables data acquisition from a DVB device.  The channel
must be tuned with an external application. The PID is the identifier
of the data stream which contains Nextview EPG data (i.e. the teletext
PID aka I<TTPID>.) The PID value must be determined by external means,
as the EPG scan does not support DVB devices. DVB support is still
B<experimental> and not officially supported.  In particular, the
EPG scan and acquisition modes which would require channel changing
are not supported.
...
Use keyword I<merged> to dump a merged database (or use code I<FF> as required
by earlier versions of nxtvepg.)  Note before you can use a merged
database you have to configure it, see L<"MERGED DATABASES">.
...
option selects the browser database). The I<-daemon> option cannot be combined
with the I<-noacq>, I<-remctrl> or other display related options.
...
terminate the daemon after the given phase has been completed for all
providers.  Phases are the same as defined in L<"ACQUISITION MODES">,
i.e. I<now>, I<near> and I<full>.  Note it's not useful to use this
option in acquisition mode I<follow-ui> because acquisition restarts
after each provider change.  This option is only useful with a fixed
list of providers.
...
programme database, then exit.  This argument must be combined
with I<-provider> to specify which database shall be exported.

To export the database in XML format, use keyword I<xml> as mode.
In this case the last mode (i.e. XMLTV DTD version) which was used
via the XMLTV export dialog is used.  To export the database in another
mode, use I<xml5>, I<xml5ltz> or I<xml6>.  For more details see
...
to write the network table of provider I<Kabel1> (CNI I<0xd92>) into
a file named I<networks.txt>, or pipe it to the paging program "more"
respectively.
...
Example:
  ...
  nxtvepg -dump html -prov d92 -outfile tv.html \
...
date and time from teletext and then terminate.  To specify from which
channel to acquire date and time use the I<-prov> option option.
(If you want to use a channel which is not an Nextview EPG provider,
you'd have to use an external application to tune the channel before you
invoke nxtvepg; with the exception of Linux' I<v4lctl setstation>
command this mode is unsupported though and may not work, depending
on the external application you're using it with.)
...
teletext header lines.  All Nextview EPG providers transmit packet 8/30/1,
and a few other networks do too (e.g. ARD and ZDF in Germany.)
...
=item B<-provscan> I<country>

When started with this argument nxtvepg will start scanning all
TV channels in the given country's frequency band for Nextview EPG
providers. Countries are specified by their top-level Internet
domain names (e.g. de, at, ch, fr, be)
When the scan is finished, nxtvepg updates the provider list stored
in the configuration file and terminates itself.
The scan is performed in the same way as described in
L<"GETTING STARTED: Search for Nextview providers"> and
L<"CONFIGURATION: Provider scan">, just without the graphical
user interface.

During the scan nxtvepg reports progress to I<stdout> unless you
redirect it into a file or pipe it into another program.
See option I<-outfile> for details.
...
nxtvepg database, this is equivalent to specifying options B<-dbdir>
and B<-provider> (if such options are provided, they are silently
...
in the "TV card input" dialog via the Configure menu (for additional
information see also L<"CONFIGURATION: TV card input">).
UNIX users can skip this section.

First you need to decide which driver to use: For TV cards with
Brooktree Bt8x8 or Philips SAA713x capture chips, the I<dsdrv> driver
should be preferred. For TV cards with Conexant CX2388x capture chips
the WDM drivers should be preferred.  Generally, the internal driver
is more reliable as it is optimized for teletext data acquisition while
the vendors' WDM drivers usually are optimized for video and sometimes
don't support teletext at all.  On the other hand, the dsdrv driver
doesn't support all TV cards and sometimes there are conflicts when
a WDM driver is installed for the same card (see also description of
option "Stop conflicting WDM drivers...")

By default nxtvepg doesn't include support for WDM. The reason for this
separation is that the use of WDM is still experimental.  Hence you need
to obtain a separate package with an interface library (VBIAcqWDMDrv.dll)
You should unpack the contents of this package into the same directory
as the nxtvepg executable.  If you don't have the library you'll see
an error message stating "Failed to load WDM interface library" when
switching to a WDM source. 
...
To start driver configuration, press the I<Configure card> button in
the middle of the "TV card input" dialog window, which will open another
...
popup stating I<"PCI scan failed">; see also the README file for more
...
=head2 Search for Nextview providers (EPG scan)
...
The TV app's channel names are used by the  I<Network name> dialog in the
...
The exact dates are printed in the date scale at the top of the window,
...
(if you enable them in L<"CONFIGURATION: Select attributes">) or pass them
to external applications (see L<"CONFIGURATION: Context menu configuration">)
...
Reception and loss statistics: Count of received EPG teletext pages and
count of complete EPG pages missing in the sequence.  Count of received
teletext packets (usually 23 per page) and missing packets.  Note: page
and packet loss is usually caused by decoding errors in packet and page
headers (which carry a 50% redundancy so this is a strong indication of
weak signal quality) or when a TV application running in parallel
interferes with capture configuration (i.e. when not all required lines
in VBI can be received)

Next is the number of received and dropped blocks.  Blocks are usually
assembled from data received in several teletext packets and have to
be dropped if packets or pages inbetween are missing. Blocks may also
be dropped due to decoding or check-sum errors (above note regarding
these errors applies here too)

The final line lists the number of decoded text characters (e.g. programme
titles and descriptions) and how many decoding errors were detected.
Forward error protection used for text is pretty weak, so you may see
errors here even with relatively good signal quality.  With bad signal
quality the actual error rate might be higher then displayed, since the
protection cannot detect double-bit errors.
...
(see L<"CONFIGURATION: Select attributes">)
...
(see L<"CONFIGURATION: Context menu configuration">).
...
I<stdout> in a free text format. This is mainly intended for developers,
...
You can also export the database in this format via the command
line by using option C<-dump raw>.  In this case the options last
used in the dialog are effective.

Note that the data can also be exported directly from the command
line by using B<-dump raw> (see L<"OPTIONS">)
...
line by using B<-dump pi>, B<-dump ai> or B<-dump pdc> (see L<"OPTIONS">)
In this case you can use one or more I<-epgquery> options to export
only selected channels and programmes.

=head2 Export as XMLTV

Open a dialog that allows to export the complete database in XML format,
as defined by the B<xmltv.dtd> version 0.5
(see L<http://xmltv.org/>)
...
Since nxtvepg version 2.9.0, XMLTV channel identifiers are exported
in the form suggested by the XMLTV DTD (i.e. a DNS-like name such as
"ard.de" for German network "ARD") so that they match the identifiers
generated by other grabbers. These names are specified in
file B<xmltv-etsi.map> which comes with nxtvepg, in sections
C<[GLOBAL]>, C<[EXPORT|GLOBAL]> and C<[EXPORT|*xyz*]> (where
"*xyz*" is a pattern which matches the provider name.) If this file
is not found, nxtvepg falls back to the old naming scheme "CNInnnn"
(where "nnnn" is the numeric hexadecimal ETSI channel ID, e.g
"CNI0D92" for "Kabel1".)  If you prefer to keep the old scheme, you
can add rules to the map file (e.g. "0DC1 CNI0DC1")
...
Note that the database can also be exported directly from the command
line by using the B<-dump xml> option (see L<"OPTIONS">). In this case,
you can additionally use one or more I<-epgquery> options to export
only selected channels and programmes (for details see L<"OPTIONS">)
...
line by using the B<-dump html> switch (see L<"OPTIONS">).  In this
case the number of written programmes can be limited by appending a
number to the keyword (e.g. "-dump html:50") and you can use one or more
I<-epgquery> options to export only selected channels and programmes.

=head2 View statistics/timescales

The following four commands are used to open windows with statistics
about the databases and acquisition progress.
See L<"STATISTICS"> for details.
...
scales get covered.
See also L<"STATISTICS: Status line">
...
of acquisition.
See also L<"STATISTICS: Timescale popup windows">.
...
the browser (e.g. when using a manual or cyclic acquisition mode, see
L<"DATA ACQUISITION">)
...
provider scan in I<refresh> mode (see L<"CONFIGURATION: Provider scan">)
You cannot remove active providers even if you're not interested in their
EPG information, because nxtvepg would add them back automatically in
the next scan or when their TV channel is tuned externally.
...
B<Note> this is still an experimental feature. Support is still
limited to German networks and configuration options are limited.
...
loading the TV overview teletext pages (e.g. 301-309 in Germany)
...
The main intention of the grabber is to improve on the data provided
by Nextview EPG.  The main disadvantage of teletext compared to
Nextview is the long time it takes to grab teletext. The cause for
...
given page are loaded, usually at least 90 seconds.  Assuming only
30 TV networks, the total load time is already 45 minutes. Another
...
cannot always work correctly.  In particular the capabilities to
...
with a Nextview EPG database (see L<"MERGED DATABASES">)

To be able to use the grabber, you have to install a programm called
"Perl" on your PC.  Perl is included within virtually every UNIX
and Linux installation, so you probably already have it if you use
one of these operating systems.  If not, you'll probably find it in
section "programming languages" on your distribution's CD or DVD.
M$-Windows users have to download and install Perl manually (from
L<http://www.activestate.com/Products/ActivePerl/?mp=1> or
L<http://www.cygwin.com/>) and then specify the path to perl in the
configuration dialog.  (Alternatively you can extend your C<PATH>
environment variable so that perl.exe is automatically found.)

By the default, the grabber is disabled since it's still experimental.
To enable it, check the box at the top of the configuration dialog.
If you haven't configured a TV application yet you'll get an error
message, since the grabber requires a TV channel table. The grabber
...
(see L<"ACQUISITION MODES">) the grabber uses an optimized
...
To use the grabbed data you have to wait until each channel has
been visited at least once. After that you should see an XML
file for each TV channel in the nxtvepg directory (named
"ttx-XXX.xml" where XXX is the TV channel name.) These files
will appear in the provider selection and merge dialogs and
can then be used just like Nextview EPG providers.
...
the internal tuner as input source.  See also L<"GETTING STARTED"> and
the subsequent chapter on TV card configuration.

If you live in France, select the French frequency table
(which implies the I<Secam L> TV norm) to be used for the EPG scan;
else use the default frequency table which covers common cable network
frequencies in Western Europe with the PAL TV norm.
Other countries are not supported, since there currently
...
VPS/PDC channel identification and up to 90 seconds if potential
EPG data packets were found.  Note that you
can change this setting while the scan is running.

Use the I<Refresh only> mode if you deleted your database files (or if you
upgrade from an incompatible software version) or if you want to identify or
...
databases and your rc/ini file). This allows to regain your complete
provider selection very quickly. If no data transmission is found on a
provider's channel, a I<Remove this provider> button will be added to the
output window, which you can use after the scan has finished to remove
the database and any information recorded about the provider.  Note that
removal will fail if the database is currently in use, so you may have to
stop acquisition and select a different provider.

Technical background information on how the EPG scan works:
...
potential EPG teletext page addresses.  If valid data packets have
...
nxtvepg web site (see I<About> in the Help menu) for a list of known
Nextview EPG providers. If you receive TV channels for which a provider
is listed, repeat the scan until they are found, e.g. at different times
of the day.  If you find new providers not listed on the web site,
please write a note in the discussion forum on the web site.
...
L<"STATISTICS: Acquisition statistics">).
...
definitions apart, see L<"CONFIGURATION: Select attributes">.
...
marked red.  If no TV application is configured, the network names
used by your currently selected EPG provider are used.
...
This file is used when importing EPG data from XMLTV file to map XMLTV
channel identification strings (i.e. the name given in the
I<E<lt>channel id="..."E<gt>> XML tags) to ETSI channel identification
numbers. If a file with the same name is in the current working directory,
that one is used instead.

The content in this file is split into sections: section C<[GLOBAL]> is
used for all XMLTV sources. Other sections are only used when the given
pattern matches the content of the C<source-info-name> or
C<source-data-url> tags in the XML file. For more details see the
comments in the file.

This file is also used when exporting EPG data into an XMLTV file (i.e.
the reverse direction of the above) to map internal numerical network
identifiers to DNS-like channel identifier names. In this case sections
C<[GLOBAL]>, C<[EXPORT|GLOBAL]> and C<[EXPORT|*xyz*]> are used and
the given pattern is matched against the provider name (not the
...
This device is used when option I<-dvbpid> is present.
...
This file is used when importing EPG data from XMLTV file to map XMLTV
channel identification strings (see UNIX section above for details or
...
are available from L<http://www.etsi.org/>
...
Many thanks
to Néo for providing the MS windows installer, the K!TV instruction
...
to Thierry Ménétrier and Peter Rolf for their valuable feedback and suggestions;
...
Copyright (C) 1999 - 2009 by T. Zoerner.
All rights reserved.
