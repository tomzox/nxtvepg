...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.61 2002/05/05 20:33:46 tom Exp tom $
# ---------------------------------------------------------------------------
...
The daemon always creates a named socket in the I</tmp> directory plus
...
parameters; else you can skip this step.
...
=head2 Search for EPG providers
...
checked for Nextview transmissions.
...
The last used options are remembered in your personal configuration file.
A history of the last searches, including options, are also stored and
available in the "Title" drop-down menu in the main browser window.
...
same host and with the same user id as the browser process.
...
currently are ignored though), which saves a lot of time.
...
If you live in France, select the French frequency table to be used
...
Here you can enable or disable logging to syslog. See I<syslogd(8)>
and I<syslog.conf(5)> for details about the syslog facility. The log
...
=item B<TV application interaction...>
...
Windows only: the I<General enable> option allows to disable or
...
The next three buttons (the first three on UNIX) allow to separately
...
your X server.) By default all three interaction features are enabled.

The I<Tune TV button> option allos to hide or show the equally named button
...
If you leave the I<Display EPG info in TV app> option enabled,
...
Many thanks to E-nek for the DScaler driver port and cooperation
...
Copyright (C) 1999, 2000, 2001, 2002 by Tom Zoerner.
All rights reserved.
