...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.97 2003/04/12 13:35:17 tom Exp tom $
# ---------------------------------------------------------------------------
...
The only way to stop the daemon is to send it a signal, i.e. kill
it with SIGTERM.  If it's running on the same host and login, it can
also be stopped by deselecting I<Enable acquisition> in the I<Control>
menu.  For more details see L<"CONTROL"> and L<"FILES">.
...
searches saved in shortcuts.  If your provider does support series
identifiers (see above), you should use those instead because the numerical
identifiers are protexted more strongly during transmission.  A text search
can fail due to a single erronous character.
...
and the I<Title> column header menu in the browser.  If you enter multiple
texts, a logical OR is used.  You can undo single text searches by
unchecking the button next to them in the history.
...
in the dialog's list).  If a match is found, the text, image or attribute which
...
drop-down menu which lists all user-defined attributes. Here you
select which attribute definition you want to edit.  Below to the left is the
...
definitions apart in the I<Attribute selection> configuration dialog.
You should also assign a column header text (only used in the "single list"
layout), which will appear in the TV schedule browser; you'll probably
use a shorter text here than in the label.

Then you add one or more shortcuts to the list by use of the I<add>
drop-down menu to the left of the listbox. You can change the order of
the shortcuts by use of the arrow buttons, or delete them again with
the I<delete> button.
...
want to unmap this button.  (It's mainly present to make users of older
versions aware of this new option.)
...
When X11 is already started you can override settings with xrdb like this:
...
Many thanks to Thierry Menetrier for his valuable feedback;
...
Copyright (C) 1999 - 2003 by T. Zoerner.
All rights reserved.
