...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.85 2002/11/19 18:57:10 tom Exp tom $
# ---------------------------------------------------------------------------
...
format (see also L<"CONTROL: Export as text">), then exit.
...
(e.g. first select format wide, then set the filter class to 2,
then select format PAL+)
...
codes; for the other ones you can use text search among programme titles.
...
There are three options: If you enable I<Start at current time>
...
the interval end is fixed to 23:59 of the same day. If you enable
I<Ignore date> the filter allows programmes that start in the given
time window on any day.
...
to use global invert for filter shortcuts.
...
or I<SHIFT> keys when selecting in the main winddow's shortcut listbox.
When combining filters with different combinations of filter categories
the behavior is slightly subtle and sometimes suprising.  That's because
currently filter shortcuts are not separated when processed.
...
described in L<"FILTERING">. For example, if one shortcut searches for
movies, another one subtitles, the combination will search for programmes
which are movies AND have subtitles. There's currently no way to omplement
a logical OR.  (This might change in the future.)
...
At the bottom of the edit menu there's a checkbutton labeled I<Hide from
main window>.  When it's enabled the currently selected shortcut will
...
columns (see L<"CONFIGURATION: User-defined columns">)
...
column configurations contains user defined columns which use markers
(see L<"CONFIGURATION: User-defined columns">), you have to put the images
into a subdirectory called F<images>; also your browser has to support
the B<PNG> image format.
...
=head2 User-defined columns

Open the configuration dialog for user-defined browser listbox columns.
This allows you to create new column types in addition to the predefined
types (e.g. "Title" and "Theme"), which can even contain images or
formatted text (i.e. bold or colored text) and have freely defined header
text and menu.  This manual section first describes the basic principles
and then the usage of the configuration dialog.

The "heart" of user-defined column definitions is a list of filter
shortcuts.  You build this list as a subset of your personal
filter shortcuts (usually displayed at the very left of the browser
window; see also L<"FILTER SHORTCUTS">).

Whenever nxtvepg displays a programme in the browser, it matches for
each user-defined column the programme's attributes against the shortctus
given for the respective column, in the given order (top to bottom in the
dialog's list).  If a match is found, the text, image or attribute which
...
In the following the usage of the configuration dialog is described:

The dialog consists of three sections: at the top to the right is a
drop-down menu which lists all previously user-defined columns. Here you
select which column definition you want to edit.  Below to the left is the
list of shortcuts and 4 buttons to edit the list.  On the right side of
the shortcut list there are various controls for defining how a shortcut
match should be displayed.

Note that there are dependencies between those sections: when you change
to a different column with the drop-down menu at the top, the rest of the
dialog is immediately updated to display the current definition of the
column (you'll get a warning if there any unsaved changes in the
previously edited column).  Also, when you select a shortcut in the
listbox, the display attribtes at the right are updated.

To create a new column, you will start by selecting I<Create new column>
in the menu at the top right. You should give the column a label right
away, so that you can keep your columns apart in the I<Column selection>
configuration dialog.  You should also assign a column header text,
which will be used in the browser; you probably will use a shorter text
here than in the label.

Then you add one or more shortcuts to the list by use of the I<add>
drop-down menu to the left of the box. You can change the order of the
shortcuts by use of the arrow buttons, or delete them again with
the I<delete> button.

By default a shortcut match is displayed as the name of the shortcut.
You can change that with the controls to the right of the listbox.
The choices are: freely defined text, one of the pre-defined images or
one of the programme's attributes (i.e. if you select I<Time> the
programme's start time will be displayed).  For textual output you can
additionally select a format below: either bold, underlined or colored.
All these display options can be configured separately for every
shortcut match.

When you want to try out your column-definition, press the I<Apply>
button at the bottom.  The column definition is saved and the browser
content is updated.  However if the column is not currently used in the
browser, nothing will happen. You have to use the I<Column selection>
configuration dialog to insert the column at your preferred position.
For newly created columns the apply function offers to automatically
append the column to the browser window.

There's also a I<Delete> button at the bottom which allows to remove
obsolete column definitions.  If the column is still displayed in the
...
of menu buttons above the programme list.
...
Can be changed with the B<-dbdir> command line option.  Note that the
path can also be configured during compilation of the software, so if
you downloaded a binary version of this package the path may be
different.  The current default setting can be queried with the
B<-help> command line switch.
...
Defaults for all officially supported options are listed in the app-defauls
file F<Nxtvepg.ad> which is usually installed in F</usr/X11/lib/X11/app-defaults>
(without the .ad extension).  You should not change valus there however,
because they would be overwritten when you install new versions.  Instead copy
changed lines into your .Xdefaults file (without the leading comment ! sign.)

Color values can specified either symbolically (e.g. "red") or as hexadecimal
RGB values in #RRGGBB format. Fonts must be specified as triplets of family,
...
Copyright (C) 1999, 2000, 2001, 2002 by T. Zoerner.
All rights reserved.
