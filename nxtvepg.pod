...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.76 2002/09/16 19:30:41 tom Exp tom $
# ---------------------------------------------------------------------------
...
UNIX users do not need to do any hardware related configuration.
However there are still 3 parameters that should be checked via
the TV card configuration dialog in the configure menu:
if you're not using an antenna cable for TV reception, choose either
Composite or S-Video as input source, depending where you've connected
your external receiver;
if you live in France and are using an antenna, you must select the
French TV frequency table (which implies using the Secam TV norm
instead of PAL B/G/I);
if you have more than one TV card you can otionally select which on to use.
...
that no frequency will be known for this provider so you have to tune
the provider's channel manually whenever you start acquisition.
