...
nxtvepg - Browsing EPG data in XMLTV format and scraping from teletext
...
format. Using a TV capture card EPG data can additionally be scraped from
TV schedule tables in teletext.
...
The I<database ...> parameters on the command line can be one or more
...
Default: On UNIX F</var/tmp/nxtvdb>, on Windows the current directory.
...
are as defined in L<"ACQUISITION MODES">, namely I<now> or I<full>.
...
Note none of this setup is required for just loading XMLTV files obtained
from other sources.
...
(for more in-depth information see also L<"CONFIGURATION">, section "TV card input").

By default nxtvepg assumes input via TV tuner. This is the preferred
mode of operation, since nxtvepg can change channels as required for
grabbing EPG.  With an external source nxtvepg can only capture EPG
from those channels you tune manually (see also L<"DATA ACQUISITION">).
...
acquisition, but afterwards expect you to tune in TV channels with
teletext at the external video source.
...
required page range was received completely. When done, it scrapes EPG
...
You can monitor acquisition progress using the "Aquisition statistics"
dialog in the Control menu. See L<"STATISTICS"> for details.



=head1 ACQUISITION MODES

The acquisition mode configuration dialog allows selecting an acquisition
...
the input source will be set; the tuner is not touched. Hence the TV
...
nxtvepg, i.e. if the TV channel scan does not find any channels with all of the
available tuner types. In this case tune in the TV channel with a
...
This default mode works straight-forward as described in chapter
...
full EPG is scraped and written. Acquisition is complete after a single
...
L<"CONFIGURATION">). The second one are the timescale popup window,
...
window which offer technical details about the database (e.g. which
percentage of entries is already loaded etc.), both in textual form and
as charts. Last but no least, there's a status window for the Teletext
EPG grabbing process.
...
file. If acquisition is working, you'll see a percentage that describes
how many of the TV channels are done and which channel is currently being
worked on.
...
tuned a station that doesn't transmit Teletext or isn't enabled in the
Teletext grabber configuration.

Else you'll see the name of the content provider and a percentage
...
The timescales are updated when the Teletext EPG grabber is running and
done with scraping EPG data for a network.
...
long ago the database was updated and the current expiry removal delay
(see L<"FILTERING: Expired Programmes Display">), some or all TV
programmes may lie in the past.  The exact dates are printed in the date
scale at the top of the window, the current time is additionally marked
with a small arrow labeled "now".

Ranges that are covered by the database are marked in shades of red or
blue; uncovered ranges in-between are marked gray. The range beyond the
last programm per network is left black. (That last range depends on the
extent of TV schedule tables in Teletext and varies widely between
networks.) The detailed meaning of colors is explained in the following
list. (However not all of these color codes are actually used anymore, as
not all are applicable to acquisition via the Teletext EPG grabber.)
...
The index in range of 0 to one less than the number of channels configured
for use by the Teletext EPG grabber. This gives the progress of
acquisition in the current cycle phase.

=item I<Channel Name:>

The name of the channel where Teletext is currently captured from. The
name is the one given in the TV channel table loaded from the configure TV
application.
...
(if you enable them in L<"CONFIGURATION">, section "Select attributes") or pass them
to external applications (see L<"CONFIGURATION">, section "Context menu configuration")
...
The first and last teletext page number which are captured for scraping
EPG for the current channel.

=item I<Captured pages:>

Number of pages received in the given range (including repetitions and
sub-pages).
...
If you compare databases of different XMLTV content providers, you will
...
several providers all the time, database merging allows selecting and
combining features or networks from several sources into one newly
created database.

When you select the B<Merge XMLTV files> entry from the B<Configure>
...
programme.  An exception are series, where only the
first provider in the list is queried (i.e. even if the first provider
does not have a series ID for a given programme, the further
...
(see above).  In this case should use those instead because the numerical identifiers have a better protection against transmission errors.  A text
...
Note that the interpretation of series identifications depends on the
provider. That's because those are
...
When starting the browser and the teletext grabber is configured,
acquisition is always automatically enabled (although failure to do so
is silently ignored). This means disabling acquisition via this menu
option is not remembered across restart of the application.
You can suppress automatic start with the B<-noacq> command line switch
or by disabling the grabber in the configuration menu.
...
UNIX: When acquisition is switched off, the TV capture device device is
freed, so that other applications can use it and tune different channels.
Acquisition can also been switched on and off automatically (i.e. from a
shell script) by sending signal C<HUP> (e.g. with the kill(1) command) to
...
of acquisition.
See also L<"STATISTICS: Timescale popup window">.
...
in the current directory.  When selecting a file in the list, the provider
and grabber names included in the XML file and the list of covered channels
are shown in the info boxes on the right side of the dialog.

When you leave the dialog with Ok, the selected XMLTV file will
be loaded into the browser.  If the file hasn't been updated for a
long time, the programme list might be empty though (i.e. all programmes
expired). In that case restart the grabber you used to create the file.
...
loading the TV overview teletext pages (e.g. 301-399 in Germany)
...
given page are loaded, usually 90 - 120 seconds.  Assuming only
30 TV networks, the total load time is more than 45 minutes. Another
...
will appear in the XMLTV file selection dialogs and can be
loaded like any other XMLTV file.
...
all XMLTV content providers. The names you specify here will be used in the
programme listbox and all filter menus, independently of the provider.

On the left side of the dialog you find a list of all networks across
all recently loaded (and still existing) XMLTV files. The names used here
...
names used by your currently selected EPG provider are used.
...
dialog.  See also L<"CONFIGURATION">, section "TV application interaction".
