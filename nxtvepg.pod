...
#  Author: Tom Zoerner
#
# $Id: nxtvepg.pod,v 1.130 2008/01/01 01:13:28 tom Exp tom $
# ---------------------------------------------------------------------------
...
(C<http://zapping.sf.net/>) for acquisition.
...
are applied.  For details see L<"CONTROL: Export as HTML">.  The
number of exported programmes can be limited by adding a number
after the keyword, separated by a colon (e.g. C<-dump html:50>.)
The format of the programme list is currently not configurable
though.
...
as defined by the B<xmltv.dtd> version 0.5 (see I<xmltv> project on
...
http://www.cygwin.com/) and then specify the path to perl in the
