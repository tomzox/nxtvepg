...
The I<Use TV app.> button is automatically checked if
a TV application was set up in the I<TV app. interaction> dialog.
In this mode nxtvepg just checks the TV channels defined in the
TV-application's channel table, which usually saves a lot of time and
is also more reliable. For DVB this is the only option.

When using an analog TV-card, if no TV-application's channel table is
configured, the scan is performed on all possible TV transmission
frequencies in multiple frequency bands.
If you live in France, select the French frequency table
