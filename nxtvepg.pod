...
nxtvepg - Browsing EPG data in XMLTV format and grabbing EPG from teletext
...
F<ttx-CHANNEL.xml>, where "CHANNEL" is the channel name from which the
data is grabbed. The file content is in XML format, using XMLTV schema.
A separate file is created for each channel from which data is grabbed.
...
use an external DVB channel scan application for that and point nxtvepg
to use its output. Currently, only the VDR format is supported, as it
is the only one that includes the PID of Teletext (e.g. unlike the one
used by mplayer).
See also L<"CONFIGURATION">, section "TV application interaction".
...
=item B<ttx-CHANNEL.xml>

Files written by the Teletext EPG grabber have names in format
F<ttx-CHANNEL.xml>, where "NAME" is the channel name from which the data is
grabbed. The file content is in XMLTV schema. A separate file is created
