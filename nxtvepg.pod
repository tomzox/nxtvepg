...
#
# $Id: nxtvepg.pod,v 1.22 2001/01/21 20:57:16 tom Exp tom $
# ---------------------------------------------------------------------------
...
<!-- end of header insert - start of body insert -->
<P><HR><P>
This is the UNIX man page for pre-release 0.4.pre0 of
<A HREF="http://nxtvepg.tripod.com/">nxtvepg</A>.
<BR>
For documentation of previous releases please refer to the
<A HREF="man-0_3_4.html">manual page from release 0.3.4</A>.
...
Nextview EPG is transmitted by the following TV stations:
...
Therefore please invoke the 'TV card input' popup from the Configure
menu and specify the type of your tuner. In Germany, Austria and
Switzerland it should be either Philips PAL or Temic PAL. In France
it's one of the SECAM types. Then select how your card's I<PLL> needs
to be set up. On most cards it's either I<No init> or I<28 MHz>.
If you're not sure about your configuration, try them one by one and
run an EPG scan inbetween to check if the tuner works.
...
UNIX users just need to check if the correct input source is selected
in the TV card input popup menu.
...
checked for Nextview transmissions.  The first one found will
automatically be loaded into the browser after the scan has stopped.
If you want a different provider, you can select it via the provider
selection popup from the Configure menu.

Alternatively if you decided not to use the tuner, provide the video
signal of one of the content provider channels listed in the introduction
to the selected input socket. See L<"DATA ACQUISITION"> about the
disadvantages of not using the TV tuner as input. As soon as an EPG
provider is recognized, its data will appear in the browser window.
...
kept busy by another application (e.g. if you watch TV) data is still
being acquired, but it's no longer possible to automatically change the
TV channel. Hence you are resposible for selecting the channel of the
...
I<red:> PI blocks received in stream 1;
I<blue:> PI blocks received in stream 2;
I<shaded red or blue:> PI blocks from an earlier database version;
I<yellow:> expired or invalid PI blocks (overlapping or zero run-time,
...
The acquiition mode configuration allows you to control for which providers
...
This is the only possible mode if you choose an input source other than
the TV tuner, since there's no means by which the TV channel that's received
through one of the composite inputs can be switched. Furthermore, this mode
is automatically used if the software detects an external, i.e. uncontrolled
channel change.
...
browser that's no on the list, no data will be loaded into the browser,
...
order and selection of networks can be configured (see I<Configure> menu).
...
pop-up, leave it with the I<Abort> button. Else, use I<Save>.
...
theme-related filters at once.

You can combine several shortcuts by pressing the I<CTRL> or I<SHIFT>
...
you can also access the popup from the I<Configure> menu.
...
I<stdout> (UNIX only) or into the given file.  Also for developers only.


=item B<View timescales>

Toggles the timescale window for the browser database.
See L<"DATA ACQUISITION"> for details.
...
=item B<Acq statistics>

Toggles the acquisition statistics window, which informs you about
the progress of the acquisition. See L<"Data Acquisition"> for details.
...
to update the database, if you have slected "Follow-UI" acquisition mode.


=item B<Merge providers...>

Open a popup that allows to merge several databases into one.


=item B<Acquisition mode...>

Open a popup that allows to control the background acquisition process.
...
most acquisition modes.


=item B<TV card input...>

Open a popup where the driver for the TV card can be configured.
On Windows systems you must at least specify which type of tuner
is on your card, and if the PLL needs to be initialized. You can
...
details see F<README>, particularily if you get error messages
from the Bt8x8 driver. If you experience data loss due to heavy
system load, you should raise the acquisition thread priority.

On Linux systems only the remaining two configuration options
apply: you may choose which video input source to use. See
L<"DATA ACQUISITION"> about the consequences of not using the
TV tuner as input.  If you have more than one TV tuner card,
you can also choose which one to use, just like with the B<-card>
command line option.
...
networks and pressing I<up> or I<down>.
...
and then close the popup with I<Save>.
...
Device files that are needed to set tuner frequency for
VBI reception.  The index postfix can be specified with the
B<-card> command line option.
The device is only kept open during a provider search. Else, it's just
opened shortly to set the input source and tuner frequency at the start
of acquisition. If the device is busy, acquisition starts on the currently
selected source and frequency and follows any externally controlled channel
changes (passive acquisition mode).
...
and shutdown mesages are catched and the driver then stopped before
the exit.
