...
#
# $Id: nxtvepg-de.pod,v 1.8 2005/08/09 19:10:46 tom Exp tom $
# ---------------------------------------------------------------------------
...
In Deutschland und Österreich: Kabel1, RTL-II (Abdeckung: ca. 32 Sender)
...
In Belgien: VT4, M6, TV5 (Abdeckung: 32 Sender)
...
Für Windows-Nutzer ist diese Option besonders hilfreich, wenn nxtvepg über die I<Autostart>-Gruppe gestartet wird. Wird I<Hide on minimize> benutzt, startet nxtvepg fast unsichtbar im Hintergrund nur mit einem Symbol im Tray der Taskbar (siehe auch L<"CONFIGURATION: Show/Hide">).
...
Tipp: Wenn der Daemon auf dem gleichen Host und der selben uid läuft, können Sie ihn auch beenden, indem Sie die Option I<Enable acquisition> unter dem I<Control>-Menü abstellen, während Sie mit dem Daemon verbunden sind. Weitere Details siehe L<"CONTROL"> und L<"FILES">.
...
Alle Ausgaben werden auf das Terminal ausgegeben (das sogenannte I<stdout>) falls sie nicht per Pipeline "|" in ein anderes Programm oder per ">" in eine Datei umgelenkt werden.  Siehe auch Option I<-outfile>.
...
Drücken Sie zunächst den I<Autodetect>-Button im rechten Teil des Dialogfensters. Falls vorhanden, wird der nichtflüchtige Speicher (EEPROM) Ihrer Karte gelesen, um den Hersteller und das Modell zu bestimmen. Im Optimalfall können so alle benötigten Parameter automatisch bestimmt werden. Klappt dies, werden alle benötigten Kartenparameter gesetzt und Sie brauchen nichts weiter zu tun, als den Konfigurations-Dialog mit OK zu schließen. Wenn Sie wollen, können Sie jedoch automatisch ermittelte Werte (z.B. Tuner-Typ) mit den unten beschriebenen Optionen überschreiben.
...
Um einen Tuner-Typen zu konfigurieren öffnen Sie das Popup-Menü "Tunerauswahl" indem Sie auf den I<Configure>-Button klicken und einen Eintrag auswählen. Bei vielen Karten steht der Tuner-Typ auf der Verpackung. Sicherer ist es, den Tuner-Typ vom silbernen Abschirmgehäuse auf der Karte selbst abzulesen.
...
Wie viele Blöcke sind insgesamt übertragen worden. Diese Zahl stammt vom AI-Block des Providers, d.h. von der Bestandsliste. Zusätzlich separat aufgelistet für Sub-Datenstrom 1 und 2 (swo)
...
Filter können entweder einmalig durch erneutes Auswählen desselben Eintrages im Filtermenü zurückgesetzt werden, oder global durch einen Klick auf den I<Reset>-Button oder -Menüeintrag.
...
Tipp: Die Senderauswahlliste kann permanent im Hauptfenster eingeblendet werden, indem eine der Checkboxen I<Show networks> im Untermenü I<Show/Hide> des Konfigurationsmenüs (siehe L<"CONFIGURATION: Show/Hide">) aktiviert wird. Außerdem sind sowohl die Senderreihenfolge als auch die Sendernamen konfigurierbar. Insbesondere können Sie auch uninteressante Sender permanent aus der Liste entfernen.
...
Wenn Sie einen Shortcut aktualisieren (I<Update>) und Ihre aktuellen Filter-Einstellungen andere Filter als Ihre zuvor gespeicherten Einstellungen enthalten, werden Sie gefragt, ob die Filtermaske automatisch angepasst werden soll (z.B. wenn Ihr Filter zuvor nach der Themenkategorie "Spielfilm" gesucht hatte, der gegenwärtige Filter jedoch aus einer Textsuche besteht). Siehe unten nach mehr Details über Filtermasken; solange Sie diese Shortcut-Maske zuvor nicht manuell bearbeitet haben, können Sie sorglos auf "Yes" drücken.
...
Der I<Delete>-Button entfernt den ausgewählten Shortcut aus der Liste (das Löschen wird das Hauptfenster nicht verändern, solange Sie nicht auf I<Save> drücken, Sie können den Dialog also mit I<Abort> verlassen, um die Aktion rückgängig zu machen).

Der I<Invoke>-Button wird die Filter-Einstellungen des Shortcuts genau so auf das Hauptfenster anwenden, als wenn Sie den Shortcut in der Liste des Hauptfensters auswählen. Sie können danach die Filter-Einstellungen über das Filtermenü ändern. Verwenden Sie den I<Update>-Button, um dem bearbeiteten Shortcut die Filter-Einstellungen des Hauptfensters zuzuweisen.

Sie sollten vorsichtig mit dem I<Update>-Button umgehen, da leicht ein falscher Shortcut überschrieben werden kann. Haben Sie solch einen Irrtum festgestellt, können Sie die Änderung rückgängig machen, entweder indem Sie den Edit-Dialog über den I<Abort>-Button verlassen oder indem Sie den entsprechenden Shortcut im Hauptfenster markieren und ihm die Originaleinstellungen über I<Update> zuweisen.
...
Das "Herz" der Attribut-Komposition ist eine Liste von Filtershortcuts und Reminder-Gruppen, die Sie als Auswahl Ihrer persönlichen Filtershortcuts und/oder Reminder-Gruppen aufbauen (siehe auch L<"FILTER SHORTCUTS"> und L<"REMINDERS">).
...
Tipp: Die Ausgabe kann auch direkt über die Befehlszeile mittels des Schalters B<-dump raw> erfolgen (siehe auch L<"OPTIONS">).  In diesem Fall werden die zuletzt im Dialog eingestellten Parameter verwendet.
...
Tipp: Die Dateien können auch direkt über die Befehlszeile mittels der Schalter B<-dump pi>, B<-dump ai> bzw. B<-dump pdc> geschrieben werden. (Siehe auch L<"OPTIONS">)
...
Tipp: Die Datenbank kann auch direkt über die Befehlszeile mittels des Schalters B<-dump xml> exportiert werden. (Siehe auch L<"OPTIONS">)
...
Oben im Dialogfenster müssen Sie den Namen der Ausgabedatei eingeben; wenn Sie Ihr keine Erweiterung geben, wird automatisch ein I<.html> angehängt. Klicken Sie auf den kleinen Ordnerbutton rechts daneben, um einen Dialog zur Dateiwahl zu offnen. Drücken Sie den I<Export>-Button, um das HTML-Dokument zu erstellen und I<Dismiss>, um das Dialogfenster zu schließen.
...
Nur Windows: Ist diese Option aktiviert, wird in der Taskbar der Eintrag des Hauptfensters entfernt, wenn dieses minimiert wird oder das Programm mit dem Befehlszeilenschalter I<-iconic> gestartet wird. Stattdessen wird ein Icon in das Systemtray der Taskbar gelegt. Ein Klick darauf mit der rechten Maustaste öffnet ein kleines Popupmenü. Die Einträge in diesem Menü haben dieselbe Bedeutung, wie die gleichnamigen im Control-Menü (siehe L<"CONTROL">).
...
Copyright (C) 1999 - 2005 by Th. Zörner. Alle Rechte vorbehalten.
