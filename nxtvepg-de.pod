...
#
# $Id: nxtvepg-de.pod,v 1.19 2009/03/14 21:24:32 tom Exp tom $
# ---------------------------------------------------------------------------
...
Aktuelle Informationen zu den Anbietern finden Sie auf der nxtvepg-Homepage im Internet (siehe "About" im Help-Menü). Falls Sie keine der oben genannten Sender empfangen, können Sie nxtvepg benutzen um EPG Dateien im XMLTV Format zu lesen.
...
Für Windows-Nutzer ist diese Option besonders hilfreich, wenn nxtvepg über die I<Autostart>-Gruppe gestartet wird. Wird "I<Hide on minimize>" benutzt, startet nxtvepg fast unsichtbar im Hintergrund nur mit einem Symbol im Tray der Taskleiste (siehe auch L<"CONFIGURATION: Show/Hide">).
...
Spezifiziert ein alternatives Verzeichnis für die Datenbanken. Vorgabe: Auf UNIX F</usr>, auf Windows das aktuelle Verzeichnis. Achtung: es besteht kein Schutz vor gleichzeitigem Schreiben durch mehrere nxtvepg-Instanzen. Deshalb sollte das Verzeichnis in das home-Verzeichnis des Benutzers gelegt werden falls mehrere TV-Karten im System oder im Netzverbund vorhanden sind.
...
Es ist wichtig zu bemerken, dass der Browser das gleiche Verzeichnis I<-dbdir> nutzen B<muss>, weil der Daemon nur Unterschiede an den im selben Verzeichnis gespeicherten Datenbank-Dateien sendet. Weitere Details siehe L<"CONFIGURATION: Client/Server">.
...
Windows-Anwender müssen zuerst den Treiber für ihre TV-Karte im Dialog "TV card input" vom Configure-Menü aus konfigurieren (weitere Informationen siehe auch L<"CONFIGURATION: TV card input">). UNIX-Anwender können diesen Abschnitt überspringen.
...
Laden der Kanaltabelle der TV-Anwendung: die Verwendung der TV-Tunerfrequenzen kann die EPG-Suche erheblich beschleunigen (wie oben bereits erwähnt). Die eingelesenen Sendernamen werden vom Dialog "I<Network name>" im Konfigurationsmenü (siehe L<"CONFIGURATION: Network names">) genutzt, um die Sendernamen zwischen nxtvepg und der TV-Anwendung zu synchronisieren.
...
Auf Windows werden verschiedene Freeware TV-Anwendungen im Zusammenhang mit den passiven Features unterstützt; dazu müssen Sie angeben welche Anwendung Sie verwenden. Sollten Sie im Dialog "I<TV card input>" die TV-Karten-Einstellungen einer TV-Anwendung geladen haben, ist der Typ der TV-Anwendung bereits konfiguriert. Ansonsten oder falls Sie eine andere Anwendung als Quelle der Kanaltabelle nutzen wollen, öffnen Sie den Dialog "I<TV app. interaction>" (siehe auch L<"CONFIGURATION: TV application interaction">).
...
Wurde ein gültiger VPS/PDC-Code (Programme Identification Label, PIL) zusammen mit der CNI empfangen, wird er nach der CNI in dekodiertem Format (z.B. DD.MM HH:MM mit Tag, Monat, Stunde und Minute) angehängt. Die VPS/PDC-Codes dienen dazu den aktuellen Programmtitel zu identifizieren. Sie können sich die Codes für alle in der Datenbank enthaltenen Sendungen anzeigen lassen (wenn Sie diese in L<"CONFIGURATION: Select attributes"> aktivieren) oder einer externen Anwendung übergeben (siehe L<"CONFIGURATION: Context menu configuration">).
...
B<Tipp:> Die Senderauswahlliste kann permanent im Hauptfenster eingeblendet werden, indem eine der Checkboxen "I<Show networks>" im Untermenü I<Show/Hide> des Konfigurationsmenüs (siehe L<"CONFIGURATION: Show/Hide">) aktiviert wird. Außerdem sind sowohl die Senderreihenfolge als auch die Sendernamen konfigurierbar. Insbesondere können Sie auch uninteressante Sender permanent aus der Liste entfernen.
...
Zusammengesetzte Attribute ("composite attribute") ersetzen einzelne Attribute in der Anzeige (z.B. nur die Spalte mit den Themenkategorien). Neu erstellte Composite-Attribute werden an die Liste der vordefinierten Attribute angehängt. Sie können dann Ihre Sendungsliste als eine Mischung aus vordefinierten und Composite-Attributen konfigurieren (siehe L<"CONFIGURATION: Select attributes">).

Composite-Attribute werden über den Dialog "I<Attribute composition>" im I<Configure>-Menü erstellt und bearbeitet (siehe L<"CONFIGURATION: Attribute composition">).
...
Die optionale, vierte Sektion ist standardmäßig leer, kann aber dazu verwendet werden, benutzerdefinierte externe Programme für die selektierte Sendung aufzurufen. Genauer gesagt können Sie auf ihrer Befehlszeile Parameter (wie beispielsweise Titel oder Startzeit) der gewählten Sendung einschließen. Die Befehle können über ein Konfigurationsmenü hinzugefügt werden (siehe L<"CONFIGURATION: Context menu configuration">).
...
Um überflüssige NexTView-Provider zu entfernen (z.B. solche, die keinen NexTView-Dienst mehr anbieten oder die nicht mehr empfangen werden), kann eine NexTView-Providersuche im I<refresh>-Modus gestartet werden (siehe L<"CONFIGURATION: Provider scan">). Aktive Provider können nicht entfernt werden, da nxtvepg sie während der nächsten Akquisition automatisch wieder einbinden würde.
...
Öffnet den Konfigurations-Dialog für "I<composite attributes>", welcher erlaubt, neue Spaltentypen für das "single list"-Layout der TV-Programmübersicht zu erstellen (siehe L<"CONFIGURATION: List layout">) beziehungsweise neue Zeilen im "spearate network columns"-Layout, die danach zusätzlich zu den vorgegebenen Attributtypen gewählt werden können (z.B. "Titel" und "Thema") wie im vorherigen Kapitel beschrieben.
...
Zuerst sollten Sie auswählen, mit welcher TV-Anwendung Sie synchronisieren wollen und wo sich die Konfigurationsdateien befinden. Dies geschieht über den Dialog "I<TV app. interaction>" im Configure-Menü (siehe L<"CONFIGURATION: TV application interaction">).
...
Sendet die angegebene Befehlszeile an eine verbundene TV-Anwendung, z.B. um den TV-Sender umzuschalten. Auch hier können sie Parametersubstitution verwenden.  Auf welchem Weg das Kommando zur TV-Anwendung übertragen wird hängt von der Anwendung ab. Wenn keine TV-Anwendung verbunden ist, erscheinen die Einträge dieses Typs ausgegraut (siehe auch L<"CONFIGURATION: TV application interaction">).
...
Versteckt oder zeigt die Checkbox "I<Grid layout>" unterhalb Shortcuts- oder Senderliste im Hauptfenster (siehe auch L<"CONFIGURATION: List layout">). Sollten Sie das eine oder das andere Layout permanent verwenden, wollen Sie womöglich diese Box verstecken (sie ist hauptsächlich präsent, um Benutzer, die von älteren Versionen upgraden, von dieser neuen Option wissen zu lassen).
...
Vielen Dank
an Néo für die fortlaufende Bereitstellung der setup.exe für Windows Releases,
...
an Thierry Ménétrier und Peter Rolf für ihr wertvolles Feedback und ihre Anregungen;
...
Copyright (C) 1999 - 2009 by Th. Zörner. Alle Rechte vorbehalten.
