...
B<nxtvepg> ist eine X11- und Windows-Anwendung zur Anzeige von Programminformationen aus Dateien in XMLTV Format. Wenn eine TV-Karte vorhanden ist, kann nxtvepg zusätzlich Programminformationen aus Teletext-Seiten ausgewählter TV-Programme extrahieren und anzeigen oder im XMLTV Format exportieren.
...
Neue Anwender sollten mit dem Lesen bei Kapitel L<"GETTING STARTED"> beginnen, das die Konfiguration des Teletext EPG Grabbers beschreibt. Falls Sie dieses Feature nicht verwenden wollen, können Sie mit Kapitel L<"BASIC BROWSING"> beginnen.
...
Zu beachten ist dass der Browser das gleiche XMLTV Datenbankverzeichnis (siehe Option I<-dbdir>) nutzen B<muss>, weil der Daemon nur Unterschiede an den im selben Verzeichnis gespeicherten Datenbank-Dateien sendet. Weitere Details siehe L<"CONFIGURATION">, Kapitel "Client/Server".
...
Im Daemon-Modus überschreibt diese Option die Akquisitionseinstellung der Konfigurationsdatei und zwingt die Akquisition in den passiven Modus (siehe L<"DATA ACQUISITION">). Die Konfigurationsdatei wird nicht geändert, sodass Sie verschiedene Akqusitionsstrategien für Daemon und GUI verwenden können.

=item B<-acqonce> I<phase>

Im Daemon-Modus hält diese Option automatisch die Akquisition an und beendet den Daemon nachdem die vorgegebene Phase für alle Provider abgeschlossen wurde. Die Phasen sind die gleichen wie unter L<"DATA ACQUISITION"> definiert, d.h. I<now> oder I<full>.
...
Um nur XMLTV Dateien aus anderen Quellen zu laden und anzuzeigen sind jedoch keinerlei Einstellungen erforderlich. Falls keine TV-Karte verfügbar ist oder Sie diese in nxtvepg nicht verwenden möchten, selektieren Sie Treiber-Type "None" im "TV-card input" Dialog im Configure Menü.
...
Bevor nxtvepg mit der Akquisition von EPG-Daten beginnt, sollte eingetragen werden welche TV-Karte und welche Quelle verwendet wird. Dies kann im Dialog "I<TV card input>" des Konfigurationsmenüs eingestellt werden (detailliertere Information siehe unter L<"CONFIGURATION">, Kapitel "TV card input").

Voreinstellung für UNIX ist das DVB Gerät mit Index 0, oder falls keine DVB-Karte vorhanden ist, das "VBI" Gerät mit Index 0. Voreinstellung für MS Windows ist das erste vom WDM Treiber gefundene Gerät (auf welche TV-Karte sich dieses bezieht ist undefiniert). Voreinstellung für die Quelle ist die erste vom Treiber gemeldete; oft ist dies der TV-Tuner, es könnte jedoch ein unbelegter externer Eingang sein.
...
Nachdem die TV-Applikation und TV-Karte konfiguriert sind, können Sie den Teletext EPG Grabber durch setzen des ersten Häkchens im entsprechenden Konfigurationsmenü einschalten. Wenn Sie den automatischen Start der Akquisition eingeschaltet lassen, wird der Grabber nach Verlassen des Dialogs durch Ok automatisch gestartet. Weitere Details siehe L<"CONFIGURATION">, Kapitel "Teletext grabber")
...
Zusätzlich kann ein Hinweis über den aktuellen Modus der Akquisition erscheinen, wie die aktuelle Phase für zyklische Akquisitionsmodi oder "I<forced passive>" wenn nxtvepg nicht in der Lage ist, den TV-Kanal zu wechseln (u.U. weil eine TV-Anwendung parallel läuft). Siehe L<"DATA ACQUISITION"> für Details.
...
Über das Control-Menü kann ein Fenster geöffnet werden welches statistische Informationen über die Browserdatenbank darstellt. Falls die Datenbank vom Teletext EPG Grabber aktualisiert wird, wird ein zusätzlicher Bereich unterhalb den Statistiken eingeblendet, in dem ein Histogramm und Statusinformationen zur Akquisition dargestellt sind.

=over 4

=item I<Last update:>

Die Uhrzeit zu welcher die Datenbank zuletzt aktualisiert wurde (basierend auf dem Modifikationszeitstempel der zugehörigen XMLTV Datei). Für zusammenfeührte Datenbanken ist dies der letzte Zeitpunkt zu dem eine der enthaltenen XMLTV Dateien aktualisiert wurde.
...
Über den Menüeintrag B<Merge XMLTV files> im B<Control>-Menü erhalten Sie einen Dialog mit zwei Listenfeldern: das Linke davon enthält eine Liste von XMLTV Dateien. Ein anderes Verzeichnis kann mit Click auf den Knopf neben dem Eingabefeld gewählt werden. Der Verzeichnisname kann auch manuell editiert werden; Nach Drücken der Return-Taste wird die Liste der Dateien aktualisiert. (Zu beachten ist dass nur Dateien mit Endung ".xml" und gültigem XMLTV Format in der Liste angezeigt werden.)

Das rechte Liste stellt die Dateien dar, die Sie zusammenführen wollen. Bei der ersten Verwendung des Dialogs wird die Liste leer sein; Bei späteren Aufrufen wird die zuletzt verwendete Auswahl wieder angezeight. Sie können Einträge in dieser Liste hinzufügen, durch Auswahl eines Eintrags in der linken Liste und Drücken des "Add" Knopfes. Einträge können aus der Lste gelöscht werden durch Auswahl eines oder mehrerer Einträge und Drücken des "Delete" Knopfes. Die Reihenfolge kann durch Auswahl von einem oder mehreren Einträgen und Drücken der beiden Pfeil-Knöpfe geändert werden. Sind Sie fertig mit Ihrer Auswahl, drücken Sie B<OK>, um das Zusammenführen der ausgeählten Dateien zu starten und die so neu erzeugte EPG Datenbank anzuzeigen.
...
Im Gegensatz zu allen anderen Filteroptionen wird diese genutzt, um den Satz zutreffender Sendungen zu I<vergrößern>. Sie ermöglicht es, abgelaufene Sendungen mit in die Liste einzuschließen (d.h. solche mit Stopzeit in der Vergangenheit); diese werden mit einem gelblichen Hintergrund gekennzeichnet (die Farbe können Sie ändern, siehe C<.Xdefaults> und C<nxtvepg.ad> in L<"FILES">). Mit dem Schieberegler können Sie die Schwelle der Ablaufzeit Minutengenau verstellen. Die Eingebefelder unterhalb des Schiebers geben die äquivalente Zeitschwelle in Einheiten von Tagen und Stunden an. Sie können diese Felder auch direkt editieren; Drücken Sie die Return-Taste um Änderungen zu übernehmen.

Standardmäßig ist die Schwelle für die Ablaufzeit Null, sodass abgelaufene Sendungen sofort aus der Anzeige verschwinden.

Beachten Sie, dass Sie nicht mehr Sendungen hinzufügen können, als in der Datenbank gespeichert sind. nxtvepg löscht abgelaufene Programme zwar nicht aus der Datenbank, jedoch passiert dies üblicherweise bei der Aktualisierung des XMLTV Files. Ob bzw. wie lange abgelaufende Programme bei Aktualisierung erhalten bleiben hängt von Ihrem Anbieter bzw. dem zur Aktualisierung verwendeten Programm ab. Beim Teletext EPG Grabber können sie den Schwellwert frei konfigurieren (siehe L<"CONFIGURATION"> Kapitel "Teletext grabber").
...
Schaltet die Teletext-EPG Datenakquisition temporär an oder aus. Diese Option ist nur wählbar wenn der Teletext EPG grabber konfiguriert wurde (siehe L<"CONFIGURATION">, Kapitel "Teletext grabber") und eine TV-Karte verfügbar ist.

Wenn Sie den Browser starten, wird die Akquisition automatisch aktiviert falls die entsprechende Option im Teletext-Grabber Konfirmationsmenü angwählt wurde. Sie können den automatischen Start temporär unterdrücken, indem Sie Befehlszeilenschalter B<-noacq> auf der Kommandozeile angeben. Das Abschalten der Akquisition erlaubt anderen Anwendungen, wie z.B. einer TV-Applikation, die TV-Karte zu nutzen und TV-Kanäle zu wechseln.
...
Öffnet einen Dialog, mit dem der Teletext EPG Grabber eingeschaltet und konfiguriert werden kann.  Um EPG Daten aus dem Teletext zu laden, werden reihum alle TV-Kanäle eingestellt, die TV-Übersichtsseiten gelesen (für deutsche Sender meist 301-399) und daraus eine Programmliste erstellt. Falls für einzelne Programme Referenzen auf weitere Teletext-Seiten angegeben sind, wird versucht von dort Beschreibungstexte zu laden.  Die Ausgabe des Teletext-Grabbers erfolgt in einzelne Dateien im XMLTV-Format. Für jeden TV-Sender wird eine getrennte XML Datei angelegt.

Ein Nachteil dieses Grabbers ist die lange Ladezeit. Diese kommt hauptsächlich dadurch zustande, dass die TV-Übersicht auf mehrere Unterseiten einzelner Teletext-Seiten verteilt sind. Dadurch werden mehrere Teletext-Zyklen für jeden TV-Sender benötigt um alle relevanten Seiten zu laden, meist mindestens 90 - 120 Sekunden. Bei Empfang via DVB ist jedoch paralleler Empfang von Sendern auf demselbem Transponder möglich, was die Ladezeit um Faktor 5 bis 8 beschleunigen kann.  Ein weiteres zentrales Problem besteht darin dass die TV-Übersicht kein einheitliches Format hat, so dass die Erkennung nicht immer zuverlässig arbeiten kann. Insbesondere können aktuell keine Themenkategorien erkannt werden.  Dieser Nachteil kann jedoch überwunden werden, indem die aus dem Teletext gewonnenen Daten anderer Quellen vereinigt werden (siehe L<"MERGED DATABASES">)
...
Dateien in diesem Verzeichnis haben Namen im Format F<ttx-SENDER.xml>, wobei "SENDER" der Name des Senders ist auf dem die EPG Daten empfangen wurden. Das Dateiformat ist XML mit XMLTV Schema. Für jeden Sender wird eine separate Datei erzeugt.
...
Nur mit DVB: Der Teletext EPG Grabber benötigt eine TV-Kanaltabelle mit den Namen, Frequenzen und Teletext PID aller empfangbaren TV Sender. nxtvepg kann die TV-Kanaltabelle nicht selber erzeugen. Sie benötigen daher eine externe Applikation die die DVB Kanalsuche durchführt und eine solche Datei erzeugt. Aktuell wird nur das VDR Format unterstützt, da nur dieses die Teletext PID enthält (im Gegensatz z.B. zu mplayer).  Siehe auch L<"CONFIGURATION">, Kapitel "TV application interaction".
...
=item B<ttx-SENDER.xml>

Vom Teletext EPG Grabber erzeugte Dateien haben Namen Format F<ttx-SENDER.xml>, wobei "SENDER" der Name des Senders ist auf dem die EPG Daten empfangen wurden. Das Dateiformat ist XML mit XMLTV Schema. Für jeden Sender wird eine separate Datei erzeugt.
