...
B<nxtvepg> S<[ options ]> S<[ databases ... ]>

B<nxtvepgd> S<[ daemon options ]>

=head1 DESCRIPTION

B<nxtvepg> ist eine X11- und Windows-Anwendung zur Anzeige von Programminformationen aus Dateien in XMLTV Format. Wenn eine TV-Karte vorhanden ist, kann nxtvepg zusätzlich Programminformationen aus Teletext verschiedener TV-Programme extrahieren und in XMLTV Format speichern.

B<nxtvepgd> ist eine reduzierte Variante von nxtvepg, die nur den Teletext EPG Grabber beinhaltet. Das Programm hat keine graphische Oberfläche, sondern arbeitet im Hinterdrund als sogenannter "daemon" Prozess. Der Prozess akquiriert kontinuierlich EPG aus Teletext und aktualisiert XMLTV Dateien, die dann im nxtveog GUI geladen werden können.

=head1 OPTIONS

Als I<database ...> Parameter auf der Kommandozeile können eine oder mehrere Dateien im XMLTV Format angegeben werden. Falls mehr als eine Datei angegeben ist, werden sie zusammengeführt wie im Kapitel L<"MERGED DATABASES"> beschrieben.  Die Daten aus den XMLTV Dateien werden dann im GUI angezeigt.
...
Im Daemon-Modus hält diese Option automatisch die Akquisition an und beendet den Daemon nachdem die vorgegebene Phase für alle Provider abgeschlossen wurde. Die Phasen sind die gleichen wie unter L<"ACQUISITION MODES"> definiert, d.h. I<now> oder I<full>.
...
Normalerweise verwendet nxtvepg den TV-Tuner-Eingang. Das ist die gängige Arbeitsweise, da nxtvepg nur so automatisch zwischen Kanälen verschiedener Teletext-Anbietern wechseln kann. Bei einer externen Quelle müßte zwischen Kanälen manuell umgeschaltet werden (siehe auch L<"DATA ACQUISITION">).

Ist kein TV-Tuner sondern ein Satellitenreceiver über den Composite- oder S-Video-Eingang im Einsatz, ist die entsprechende Einstellung im "I<video input>"-Dropdown-Menü auszuwählen. Schließen Sie den Dialog mit OK und öffnen Sie den Dialog für die Konfiguration des Akquisitionsmodus im selben Menü. Dort sollte in den Modus I<external> gewechselt werden: In diesem Modus schaltet nxtvepg beim Start der Akquisition zum eingestellten Video-Eingangskanal der externen Video-Quelle um. Danach müssen Sie die Kanäle manuell zu den TV-Sendern umstellen von denen sie EPG Daten laden wollen.

Obwohl es möglich ist, den Satellitenreceiver über ein Antennenkabel anzuschließen, ist von dieser Variante abzuraten, da das Signal oft so stark beeinträchtigt wird, dass nxtvepg den EPG-Datenstrom nicht mehr dekodieren kann.
...
Laden der Kanaltabelle der TV-Anwendung: die Verwendung der TV-Tunerfrequenzen aus einer externen Kanaltabelle ist Voraussetzung für die Verwendung des Teletext Grabbers. Die eingelesenen Sendernamen werden außerdem vom Dialog "I<Network name>" im Konfigurationsmenü (siehe L<"CONFIGURATION">, Kapitel "Network names") genutzt, um die Sendernamen zwischen nxtvepg und der TV-Anwendung zu synchronisieren.
...
Das Grundprinzip des Teletext EPG Grabbers ist der Reihe nach alle in der Kanaltabelle gefundenen TV-Kanäle einzustellen und deren Teletext zu laden. Für jeden Kanal dessen Teletext geladen wurde werden die Seiten danach analysiert und EPG Informationen extrahiert, um diese in Dateien im XMLTV Format zu speichern.

Programminformationen werden im Browser aktualisiert sobald eine XMLTV Datei aktualisiert wurde. Dabei wird versucht die Cursorposition bzw. Titelwahl nicht zu verändern, es sei denn, der Cursor steht auf dem allerersten Eintrag - in diesem Fall wir der Cursor auf das gegebenenfalls neue ersten Programm in der Datenbank gesetzt.
...
TV-Programmtabellen in Teletext werden üblicherweise nur einmal täglich aktualisiert (es sei den es treten aktuelle Programmänderungen auf). Von daher ist es ausreichend die Akquisition einmal täglich laufen zu lassen.
...
Es gibt eine Reihe von Optionan, Informationen zum Status der Datenbank und des Akquisitions-Prozesses zu erhalten: Der Erste und prägnanteste ist die Statuszeile am unteren Rand des Hauptfensters (nur wenn diese auch aktiviert ist, siehe L<"CONFIGURATION">). Der Zweite besteht aus dem Zeitskalen-Fenster ("timescale"), welche die mit TV-Programmdaten belegten Zeitspannen jedes TV-Senders visualisiert. Der Dritte besteht aus der Datenbankstatistik, welche technische Details über die im Browser geladene Datenbank liefern (wie z.B. welcher Prozentsatz an Einträgen bereits aktualisiert ist usw.). Weiterhin gibt es ein Fenster das Status und Historie des EPG Grabbers wiederspiegelt.
...
Zur Browserdatenbank sehen Sie normalerweise den Namen der XMLTV Datei. Falls die Akquisition aktiv ist, wird der Prozentsatz der erledigten TV-Sender und der Name des aktuell bearbeiteten Senders angezeigt.

Liegen mehr als 10% der Einträge der Programmdatenbank in der Vergangenheit, werden Sie zusätzlich einen Hinweis über den Prozentsatz der abgelaufenen Einträge sehen. Beachten Sie, dass eine zu 100% geladene Datenbank komplett leer erscheinen kann, wenn alle enthaltenen Programme abgelaufen sind. Sobald Sie die Akquisition starten, wird der prozentuale Füllgrad auf 0 fallen, weil ein neuer Bestand geladen wird, der diese verfallenen Blöcke nicht mehr enthält.
...
Ist die Akquisition derzeit nicht aktiv, wird darauf hingewiesen. Gegebenenfalls erscheint auch ein Hinweis auf den Grund (z.B. "no reception" oder "forced passive" wenn der Senderwechsel durch eine andere Applikation blockiert wird.)
...
Das Zeitskalenfenster spiegelt für jeden TV-Sender wieder, welche Zeitspannen mit TV-Programminformationen in der Datenbank belegt sind. Arbeitet die Akquisition an der Datenbank, können Sie die Fortschritt bzw. den Aktualisierungsgrad der TV-Programmdatenbank beobachten.

Das Fenster besitzt eine Skala für jeden TV-Sender, der von der geladenen XMLTV Datei abgedeckt wird. Das linke Ende der Skalen verweist auf die Startzeit der ältesten in der Datenbank vorhandenen TV-Sendung. Abhängig davon wie lange das letzte Update der Datenbank zurückliegt und abhängig von der eingestellten Verzögerung bis zur automatischen Löschung abgelaufener Programme (siehe L<"FILTERING: Expired Programmes Display">), können die Sendungen teilweise oder sogar ganz in der Vergangenheit liegen. Die exakten Datierungen werden in der Datumsskala am oberen Rand des Fensters ausgegeben, wobei die aktuelle Zeit zusätzlich durch einen mit "now" gekennzeichneten kleinen Pfeil markiert ist.

Bereiche die von Sendungen des jeweiligen Senders in der Datenbank belegt werden, sind durch rote oder blaue Farbe markiert, nicht belegte Teilbereiche sind grau. Der Bereich jenseits des letzten bekannten Programms ist schwarz. (Die Länge der unbelegten Bereiche unterscheidet sich für jeden Sender da diese direkt mit der Länge der Programmübersicht im Teletext abhängen.) Die Bedeutung der unterschiedlichen Farben sind in der folgenden Liste beschrieben. (Aktuell werden nicht mehr alle dieser Farben verwendet, da nicht alle für den Teletext-EPG grabber anwendbar sind.)
...
Zeitspanne die vom Inhaltsanbieter nicht abgedeckt wird.
...
=item I<Channel Index:>

Index des Senders innerhalb der Sendertabelle an dem der Teletext-EPG grabber gerade arbeitet.

=item I<Channel Name:>

Name des Senders innerhalb der Sendertabelle an dem der Teletext-EPG grabber gerade arbeitet.
...
=item I<Captured range:>

Nummern der ersten und letzten Teletextseite die vom aktuell bearbeiteten Sender geladen werden.

=item I<Captured pages:>

Anzahl der bereits geladenen Seiten (inklusive Wiederholungen und Unterseiten).
...
Blendet das Statistikfenster für die Browser-Datenbank ein oder aus (d.h. für den Anbieter dessen Daten Sie aktuell im Hauptfenster verwenden).  Die Statistik gibt Auskunft über die Anzahl der Programmeinträge in der Datenbank, den prozentualen Füllungsgrad, den Prozentsatz der abgelaufenen Einträge, das Datum des letzten Updates usw.  Läuft die Akquisition auf der gleichen Datenbank, enthält es zusätzlich Information zu Status und Fortschritt der Akquisition. Für Details siehe L<"STATISTICS: Timescale popup window">.
...
Auf der linken Seite des Dialogs finden Sie eine Liste aller Sender jeder zuvor geladenen XMLTV Datei (soweit die Datei noch existiert). Die hier benutzten Namen sind jene, welche Sie zuvor konfiguriert haben, oder falls Sie dies noch nicht getan haben, welche am besten auf die Stationsnamen Ihrer TV-Anwendung zutreffen. Nicht zutreffende Namen sind rot markiert. Ist keine TV-Anwendung konfiguriert, werden die von der aktuell geladenen XMLTV Datei gewählten Namen übernommen.
...
Sektionen C<[EXPORT|...]> werden für die umgekehrte Richtung verwendet, d.h. den Export von EPG Daten in XMLTV Dateien. Details dazu siehe L<"CONTROL: Export as XMLTV">.
