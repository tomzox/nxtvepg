 ...
 *
 *  $Id: cmdline.c,v 1.2 2005/01/08 15:16:08 tom Exp tom $
 */
...
static void MainOptionError( const char *argv0, const char *argvn, const char * reason )
{
   ...
   sprintf(pBuf, pUsageFmt, argv0, reason, argvn, argv0);
   MessageBox(NULL, pBuf, "Nextview EPG Decoder Options Error", MB_ICONSTOP | MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
   xfree(pBuf);
#endif
...
static void Usage( const char *argv0, const char *argvn, const char * reason )
{
   ...
   MessageBox(NULL, pBuf, "Nextview EPG Decoder Usage", MB_ICONSTOP | MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
   xfree(pBuf);
#endif
...
static void CmdLine_SetOutfile( void )
{
   ...
   {
      fd = open(mainOpts.pStdOutFileName, O_CREAT|O_EXCL|O_WRONLY, 0666);
      if (fd != -1)
      {
         if (dup2(fd, 1) != -1)
         {
         ...
         {
            MainOptionError(mainOpts.pOptArgv0, strerror(errno), "failed to install outfile as stdout");
         }
         close(fd);
      }
      else
...
// ---------------------------------------------------------------------------
// Switch into daemon mode if executable name has "d" postfix
//
static bool CmdLine_CheckDaemon( const char * pArgv0 )
{
   const char * pName;
   bool  result;

   pName = strrchr(pArgv0, '/');
   #ifdef WIN32
   if (pName == NULL)
      pName = strrchr(pArgv0, '\\');
   #endif
   ...
   result = (strncmp(pName, "nxtvepgd", 8) == 0);
...
//
static bool CmdLine_GetDumpMode( const char * pModeStr )
{
...
static void CmdLine_Parse( int argc, char * argv[] )
{
   ...
   while (argIdx < argc)
   {
      if (argv[argIdx][0] == '-')
      {
         ...
         {  // do not enable acquisition
            if (argIdx + 1 < argc)
            {
               if (!strcmp(argv[argIdx + 1], "full"))
                  ...
                  MainOptionError(argv[0], argv[argIdx + 1], "unknown mode keyword for -acqonce: expecting now, near or all");
               argIdx += 2;
            }
            else
         ...
         {  // dump database and exit
            if (argIdx + 1 < argc)
            {
               ...
               {
                  CmdLine_GetDumpMode(argv[argIdx + 1]);
                  if (mainOpts.optDumpMode == EPG_DUMP_NONE)
            ...
            {  // read hexadecimal CNI of selected provider
               ...
               else
               {
                  ...
                  if (pe != (argv[argIdx + 1] + strlen(argv[argIdx + 1])))
                     MainOptionError(argv[0], argv[argIdx+1], "invalid provider CNI (must be hexadecimal, e.g. 0x0d94 or d94)");
               }
               argIdx += 2;
            }
            else
         ...
         {  // extract current time from teletext
            if (argIdx + 1 < argc)
            {
               ...
               if ( (strcasecmp("de", argv[argIdx + 1]) == 0) ||
                  ...
                  MainOptionError(argv[0], argv[argIdx + 1], "illegal mode keyword for -provscan");
               argIdx += 2;
            }
            else
               Usage(argv[0], argv[argIdx], "missing mode keyword after");
         ...
         else if (!strcmp(argv[argIdx], "-demo"))
         {
            ...
            {  // save file name of demo database
               ...
               if (stat(mainOpts.pDemoDatabase, &st) != 0)
               {
         ...
         {  // undocumented option (internal use only): pass fd of pipe to GUI for daemon start
            ...
            {  // read decimal fd (silently ignore errors)
               if (!strcmp(argv[argIdx + 1], "quit"))
                  mainOpts.optRemCtrl = UWM_QUIT;
               else if (!strcmp(argv[argIdx + 1], "raise"))
                  mainOpts.optRemCtrl = UWM_RAISE;
               else if (!strcmp(argv[argIdx + 1], "iconify"))
                  mainOpts.optRemCtrl = UWM_ICONIFY;
               else if (!strcmp(argv[argIdx + 1], "deiconify"))
                  mainOpts.optRemCtrl = UWM_DEICONIFY;
               else if (!strcmp(argv[argIdx + 1], "acqon"))
                  mainOpts.optRemCtrl = UWM_ACQ_ON;
               else if (!strcmp(argv[argIdx + 1], "acqoff"))
                  mainOpts.optRemCtrl = UWM_ACQ_OFF;
               else
      ...
      {  // database file argument -> determine dbdir and provider from path
         EpgDbDumpGetDirAndCniFromArg(argv[argIdx], &mainOpts.dbdir, &mainOpts.startUiCni);
         if (mainOpts.startUiCni == 0)
   ...
   if (IS_DAEMON(mainOpts) || IS_CLOCK_MODE(mainOpts))
   {
      if (mainOpts.disableAcq)
         MainOptionError(argv[0], "-noacq", "Cannot be used in daemon/clock mode");
      else if (IS_DUMP_MODE(mainOpts))
         MainOptionError(argv[0], "-dump", "Cannot be used in daemon/clock mode");
      #ifdef WIN32
      else if (mainOpts.optRemCtrl != 0)
         MainOptionError(argv[0], "-remctrl", "Cannot be used in daemon/clock mode");
      #endif
      else if ((mainOpts.startUiCni != 0) && mainOpts.optAcqPassive)
         MainOptionError(argv[0], "-provider", "Useless together with -acqpassive in daemon/clock mode");
   }
   else
   #endif
   {
   ...
   if (IS_DUMP_MODE(mainOpts) && (mainOpts.startUiCni == 0))
...
static void CmdLine_SetDefaults( bool daemonOnly )
{
   ...
   mainOpts.optDaemonMode = (daemonOnly ? DAEMON_RUN : EPG_GUI_START);
   mainOpts.optDumpMode = EPG_DUMP_NONE;
...
void CmdLine_Process( int argc, char * argv[], bool daemonOnly )
{
   ...
   if (CmdLine_CheckDaemon(argv[0]))
   {
      // can be overridden with daemonstop option later
      mainOpts.optDaemonMode = DAEMON_RUN;
   }
