 ...
 *  Author: Tom Zoerner
 *
 *  $Id: rcfile.c,v 1.10 2007/02/25 20:33:34 tom Exp tom $
 */
...
static const RCPARSE_CFG rcParseCfg_ttx[] =
{
   ...
   { RC_TYPE_STR,  RC_OFF(ttx.perl_path_win), "perl_path_win" },
};
static const RCPARSE_CFG rcParseCfg_tvapp[] =
{
   { RC_TYPE_INT,  RC_OFF(tvapp.tvapp_win), "tvapp_win" },
   { RC_TYPE_STR,  RC_OFF(tvapp.tvpath_win), "tvpath_win" },
   { RC_TYPE_INT,  RC_OFF(tvapp.tvapp_unix), "tvapp_unix" },
   { RC_TYPE_STR,  RC_OFF(tvapp.tvpath_unix), "tvpath_unix" },
};
   ...
   else if (sectIdx < RC_ARR_CNT(rcParseCfg))
   {
      if (rcParseCfg[sectIdx].flags & RCPARSE_ALLOC)
      {
         if (sbuf[0] != 0)
         {
            ...
            RcFile_AssignParam(rcParseCfg[sectIdx].pList, rcParseCfg[sectIdx].listLen,
                               sbuf, RcFile_DynListGetPtr(pDynList));
         }
      }
