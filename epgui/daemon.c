 ...
 *  Author: Tom Zoerner
 *
 *  $Id: daemon.c,v 1.4 2005/01/08 15:16:21 tom Exp tom $
 */
   ...
   if (mainOpts.optNoDetach)
   {
      ...
      if (pAttachConsole != NULL)
      {
         if ( pAttachConsole() )
         {
            ...
            if (!SetConsoleTitle("Nextview EPG Decoder Daemon"))
...
void Daemon_Start( void )
{
   ...
   if (EpgSetup_DaemonAcquisitionMode(mainOpts.startUiCni, mainOpts.optAcqPassive, mainOpts.optAcqOnce))
   {
      #ifndef WIN32
      Daemon_SignalSetup();
      #endif
