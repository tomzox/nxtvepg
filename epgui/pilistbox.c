...
bool        pibox_open = FALSE;  // listbox is open and active
...
static void PiListBox_InsertItem( const PI_BLOCK *pPiBlock, int pos )
{
   ...
   strftime(start_str, 19, "%a %d.%m %H:%M", localtime(&pPiBlock->start_time));
   ...
   {
      strncpy(netname, AI_GET_NETWOP_NAME(pAiBlock, pPiBlock->netwop_no), 6);
      netname[6] = 0;
   }
   else
   {
   ...
   sprintf(comm, ".all.pi.list.text insert %d.0 {%-6s\t%s-%s %c%c\t%s\n} %s\n",
           pos + 1,
           netname,
           start_str, stop_str,
...
{  
   ...
   {
      EpgDbLockDatabase(dbc, TRUE);
      pPiBlock = EpgDbSearchPi(dbc, pPiFilterContext, pibox_list[pibox_curpos].block_no, pibox_list[pibox_curpos].netwop_no);
      if (pPiBlock != NULL)
      {
	 ...
	 if ((delta > 0) && (pibox_count >= PIBOX_HEIGHT))
	 {
	    ...
	    if (pPiBlock != NULL)
	    {
		  ...
		  PiListBox_AdjustScrollBar();
	       ...
	       else if (pibox_curpos < PIBOX_HEIGHT - 1)
...
void PiListBox_DbInserted( const EPGDB_CONTEXT *usedDbc, const PI_BLOCK *pPiBlock )
{
   ...
   if (usedDbc != dbc)  // UI database modified?
      return;
   ...
   EpgDbLockDatabase(dbc, TRUE);
   if (pibox_open && EpgDbFilterMatches(dbc, pPiFilterContext, pPiBlock))
   {
      if ((pibox_count >= PIBOX_HEIGHT) &&
...
void PiListBox_DbRemoved( const EPGDB_CONTEXT *usedDbc, const PI_BLOCK *pPiBlock )
{
   ...
   if (pibox_open && (usedDbc == dbc) && (pibox_count > 0))
   {
      for (pos=0; pos < pibox_count; pos++)
      {
	 ...
	 dprintf7("remove item pos=%d (net=%d,block=%d) cur=%d,off=%d,max=%d,c=%d\n", pos, pibox_list[pos].netwop_no, pibox_list[pos].block_no, pibox_curpos, pibox_off, pibox_max, pibox_count);
	 ...
	 pTemp = EpgDbSearchPi(dbc, pPiFilterContext, pibox_list[0].block_no, pibox_list[0].netwop_no);
	 if (pTemp != NULL)
         {
	    ...
	    debug2("PiListBox-DbRemoved: first listed block=%d netwop=%d not found\n", pibox_list[0].block_no, pibox_list[0].netwop_no);

	 pTemp = EpgDbSearchPi(dbc, pPiFilterContext, pibox_list[pibox_count - 1].block_no, pibox_list[pibox_count - 1].netwop_no);
	 if (pTemp != NULL)
         {
	    pNext = EpgDbSearchNextPi(dbc, pPiFilterContext, pTemp);
            if (pNext == pPiBlock)
               pNext = EpgDbSearchPrevPi(dbc, pPiFilterContext, pNext);
         }
	 else
	    debug2("PiListBox-DbRemoved: last listed block=%d netwop=%d not found\n", pibox_list[pibox_count - 1].block_no, pibox_list[pibox_count - 1].netwop_no);
	 ...
	 EpgDbLockDatabase(dbc, FALSE);
      ...
      {
	 dprintf2("do not remove item off=%d,max=%d\n", pibox_off, pibox_max);
	 pibox_resync = TRUE;
      }
   }
}
...
void PiListBox_DbRecount( const EPGDB_CONTEXT *usedDbc )
{
   ...
   if (pibox_open && (usedDbc == dbc) && pibox_resync)
   {
      pibox_resync = FALSE;
